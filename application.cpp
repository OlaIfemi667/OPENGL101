...
int main(void) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // float greenValue = (sin(timevalue) / 2.0f)  + 0.5f; // on veut modifier
    // la valeur en controlant la valeur de G dans RGBA float redValue =
    // (cos(timevalue) / 2.0f) + 0.5f; float blueValue = (acos(timevalue)
    // / 2.0f) + 0.5f; Notons que la localisation de la valeur "uniform" e
    // requiert pas l'utilisation du shader int vertexColorLocation =
    // glGetUniformLocation(shaderProgram, "ourColor");

    shader1.use();
    shader1.setFloat("offset", 0.1f);

    // Notons également que la MAJ d'un "uniform" se fait apres utilisation du
    // shader (glUseProgram)
